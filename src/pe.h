...
namespace XKLib
{
    namespace PE
    {
        ...
        namespace IMAGE
        {
            constexpr inline auto NUMBEROF_DIRECTORY_ENTRIES = 16;
            constexpr inline auto DIRECTORY_ENTRY_EXPORT     = 0;
            constexpr inline auto SIZEOF_SHORT_NAME          = 8;
            constexpr inline auto FILE_MACHINE_I386          = 0x14c;
            constexpr inline auto FILE_MACHINE_IA64          = 0x200;
            constexpr inline auto FILE_MACHINE_AMD64         = 0x8664;
            ...
            {
                std::uint8_t Name[SIZEOF_SHORT_NAME];
            ...
            struct OPTIONAL_HEADER
            {
                ...
                DATA_DIRECTORY
                DataDirectory[NUMBEROF_DIRECTORY_ENTRIES];
            };

            struct PARENT_NT_HEADERS
            {
            ...
            template <IntType T>
            struct NT_HEADERS : PARENT_NT_HEADERS
            {
                OPTIONAL_HEADER<T> OptionalHeader;

                constexpr inline auto first_section() const
                {
                    ...
                    struct NT_HEADERS32
                    {
                        std::uint32_t Signature;
                        FILE_HEADER FileHeader;
                        OPTIONAL_HEADER<std::uint32_t> OptionalHeader;
                    };
                    ...
                    struct NT_HEADERS64
                    {
                        std::uint32_t Signature;
                        FILE_HEADER FileHeader;
                        OPTIONAL_HEADER<std::uint64_t> OptionalHeader;
                    };

                    if constexpr (std::is_same<std::uint32_t, T>::value)
                    {
                        return view_as<SECTION_HEADER*>(
                          view_as<std::uintptr_t>(this)
                          + offsetof(NT_HEADERS32, OptionalHeader)
                          + FileHeader.SizeOfOptionalHeader);
                    }

                    return view_as<SECTION_HEADER*>(
                      view_as<std::uintptr_t>(this)
                      + offsetof(NT_HEADERS64, OptionalHeader)
                      + FileHeader.SizeOfOptionalHeader);
                }

                auto rva_2_raw(
                  const std::uint32_t rva,
                  std::optional<IMAGE::SECTION_HEADER**> sectionHeaderFound = std::
                    nullopt) const -> std::uint32_t
                {
                    const auto section_header = first_section();
                ...
                auto find_exported_function(
                  const DOS_HEADER* const dosHeader,
                  const std::string& funcName,
                  const auto baseAddress,
                  const std::function<
                    auto(const std::string&, const std::string&)
                      ->module_sym_t> handleForwardedFunction) const
                  -> module_sym_t
                {
                    const auto exp_dir_entry = &OptionalHeader.DataDirectory
                                                  [IMAGE::
                    ...
                    {
                        /**
                         * If mapped, then we apply directly the relative
                         * virtual address
                         */
                        if constexpr (M)
                        {
                    ...
                    const auto export_directory = view_as<
                      const IMAGE::EXPORT_DIRECTORY* const>(
                      get_va(entry_rva));
                    ...
                    /* dll.function */
                    const auto do_forwarded_function =
                      [&](const auto func_ptr)
                    {
                        ...
                        {
                            const auto module_name = forward_func_name
                                                       .substr(0,
                                                               dot_pos
                                                                 + 1);

                            const auto real_func_name = forward_func_name
                                                          .substr(
                                                            dot_pos + 1,
                                                            forward_func_name
                                                              .size());
