...
communicate_read_header(struct task_struct *task, ptr_t address)
{
	...
	if (c_copy_from_user(task, address, &communicate_header,
...
enum communicate_error communicate_read__write_struct(
	struct task_struct *task, ptr_t address,
	struct communicate_write_struct *communicate_write)
{
	if (c_copy_from_user(task, address, communicate_write,
			     sizeof(struct communicate_write_struct))) {
		c_printk(
			"couldn't read communicate write struct from task %i\n",
			task->pid);
...
communicate_read__read_struct(struct task_struct *task, ptr_t address,
			      struct communicate_read_struct *communicate_read)
{
	if (c_copy_from_user(task, address, communicate_read,
...
enum communicate_error communicate_process_cmd_read(struct task_struct *task,
						    ptr_t address)
{
	...
	error = communicate_read__read_struct(task, address, &communicate_read);
	...
	if (c_copy_from_user(remote_task, temp_buffer.addr,
	...
	if (c_copy_to_user(task, (ptr_t)communicate_read.vm_local_address,
			   (ptr_t)temp_buffer.addr, temp_buffer.size)) {
...
enum communicate_error communicate_process_cmd_write(struct task_struct *task,
						     ptr_t address)
{
	enum communicate_error error;
	struct communicate_write_struct communicate_write;
	...
	error = communicate_read__write_struct(task, address,
					       &communicate_write);
	...
	if (c_copy_from_user(task, temp_buffer.addr,
	...
	struct communicate_header_struct *communicate_header)
{
	...
	for (cmd_number = 0; cmd_number < communicate_header->number_of_cmds;
		...
		case COMMUNICATE_CMD_READ: {
			error = communicate_process_cmd_read(
				task, cmd_header->address);
			break;
		}
		case COMMUNICATE_CMD_WRITE: {
			error = communicate_process_cmd_write(
				task, cmd_header->address);
			break;
		}
